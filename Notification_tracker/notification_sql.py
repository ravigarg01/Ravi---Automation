...
credentials = ServiceAccountCredentials.from_json_keyfile_name(
  'D:\Ravi_projects\Scraping\credentials\credentials.json', scope)
    ...
    while True:
      ...
      while current_row <= row_count:
        ...
        try:
          ...
          # update sheet with date and crawl status at once using batch update
          ...
          notfication_sheet.append_row(list(data.values()))
          sheet.update_acell('D' + str(current_row), 'FALSE')
          sheet.update_acell('E' + str(current_row), data['time'] + " " + data['date']) 
          google_api_count += 1
          if google_api_count >= 59:
            print("Google API limit reached")
            time.sleep(60)
